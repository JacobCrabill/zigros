...
RUN wget -qO- https://ziglang.org/download/0.15.1/zig-linux-x86_64-0.15.1.tar.xz | tar xJv
RUN wget -qO- https://github.com/zig-robotics/zigros/archive/refs/heads/main.tar.gz | tar xzv
RUN cd zigros-main; /zig-linux-x86_64-0.15.1/zig build -Doptimize=ReleaseFast -Dtarget=x86_64-linux-gnu -Dlinkage=dynamic --summary none
...
RUN cd rclcpp_example-main; /zig-linux-x86_64-0.15.1/zig build -Doptimize=ReleaseFast -Dlinkage=dynamic -Dtarget=x86_64-linux-gnu --summary none
